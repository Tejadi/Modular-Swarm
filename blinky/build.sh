#!/usr/bin/env bash
#
# Build the blinky sample for the Seeed XIAO nRF52840 using the
...
#   ./build.sh --sense         # build for the XIAO BLE Sense variant
#   ./build.sh --flash         # build, then copy the UF2 to a mounted XIAO drive
#
# Flags can be combined, e.g. ./build.sh --pristine --sense --flash
set -euo pipefail
...
for arg in "$@"; do
  case "$arg" in
    ...
    --sense)    BOARD="xiao_ble/nrf52840/sense" ;;
    --flash)    DO_FLASH=1 ;;
...
UF2="build/blinky/zephyr/zephyr.uf2"
...
if [ "$DO_FLASH" -eq 1 ]; then
  ...
  cp "$UF2" "$DRIVE/"
  echo ">>> Done. The board will reboot and start blinking."
fi
